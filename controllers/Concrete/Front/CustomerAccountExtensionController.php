...
class CustomerAccountExtensionController extends ModuleFrontController
{
    ...
    public function initContent() : void
    {
        ...
        if ($this->context->customer->id) {
            $storedCards = CheckoutAPI::get()->hostedTokenization($this->context->shop->id)->getTokens($this->context->customer->id)->toArray();
        }
        $this->context->smarty->assign(['module' => $this->module->name, 'stored_cards' => $storedCards, 'img_path' => \rtrim($this->context->shop->getBaseURL(), '/') . $this->module->getPathUri() . 'views/img/']);
    ...
    {
        parent::setMedia();
        $this->registerStylesheet($this->module->name . '-storedcards', $this->module->getPathUri() . 'views/css/storedcards.css', ['server' => 'remote']);
    }
    ...
    public function deleteCard() : bool
    {
        ...
        $tokenCalculated = Tools::getToken(\true, $this->context);
        if (!$idStoreCard || !$tokenSent || $tokenSent != $tokenCalculated) {
            $this->errors[] = $this->module->l('Could not delete stored card.', 'storedcards');
            return \false;
        }
        $response = CheckoutAPI::get()->hostedTokenization($this->context->shop->id)->deleteToken($this->context->customer->id, $idStoreCard);
        if ($response->isSuccessful()) {
            $this->success[] = $this->module->l('Card deleted successfully.', 'storedcards');
            $this->redirectStoredCards = \true;
            return \true;
        }
        $this->errors[] = $this->module->l('Could not delete stored card.', 'storedcards');
        return \false;
