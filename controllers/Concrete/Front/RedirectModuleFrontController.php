...
{
    public const ACTIONS = ['redirectReturnHosted', 'redirectReturnIframe', 'redirectReturnInternalIframe', 'redirectReturnPaymentLink'];
    /**
     * @return array
     */
    ...
    public function displayAjaxRedirectExternal()
    {
        ...
        $response = CheckoutAPI::get()->hostedCheckout((string) $this->context->shop->id)->createSession(new HostedCheckoutSessionRequest(ServiceRegister::getService(CartProvider::class), $this->context->link->getModuleLink($this->module->name, 'redirect', ['action' => 'redirectReturnHosted']), $productId ? PaymentProductId::parse($productId) : null, \Tools::getValue('tokenId', null)));
        if (!$response->isSuccessful()) {
            \Tools::redirect($this->context->link->getPageLink('order', null, null, ['step' => 3, $this->module->name . 'DisplayPaymentTopMessage' => 1]));
        }
     ...
     */
    public function dieOrderStep3($displayErrorMessage = \true)
    {
        $params = ['step' => 3];
        if (\true === $displayErrorMessage) {
            $params[$this->module->name . 'DisplayPaymentTopMessage'] = 1;
        }
        die(\json_encode(['redirectUrl' => $this->context->link->getPageLink('order', null, null, $params)]));
    }
    /**
    ...
    public function displayAjaxRedirectReturnInternalIframe()
    {
        ...
        $response = CheckoutAPI::get()->payment((string) $this->context->shop->id)->getPaymentOutcome(PaymentId::parse($paymentId), \Tools::getValue('RETURNMAC', null));
        if (!$response->isSuccessful()) {
            OnlinePaymentsPrestaShopUtility::dieJsonArray(['success' => \false, 'message' => $this->module->l('An error occurred while fetching the payment status.', 'payment')]);
        }
        $cart = new \Cart((int) $response->getPaymentTransaction()->getMerchantReference());
        if (\false === \Validate::isLoadedObject($cart) || (int) $cart->id_customer !== (int) $customer->id) {
            $this->dieOrderStep3();
        }
        if ($response->isWaiting()) {
            OnlinePaymentsPrestaShopUtility::dieJsonArray(['success' => \true, 'waiting' => \true]);
        }
        if (\false !== \Order::getIdByCartId((int) $response->getPaymentTransaction()->getMerchantReference())) {
            OnlinePaymentsPrestaShopUtility::dieJsonArray(['success' => \true, 'waiting' => \false, 'redirectUrl' => $this->context->link->getPageLink('order-confirmation', null, null, ['id_cart' => $cart->id, 'id_module' => $this->module->id, 'key' => $customer->secure_key])]);
        }
    ...
    public function displayAjaxRedirectReturnPaymentLinkInternalIframe()
    {
        ...
        $merchantReference = \Tools::getValue('merchantReference', '');
        $response = CheckoutAPI::get()->payment((string) $this->context->shop->id)->getPaymentOutcome(null, null, $merchantReference);
        if (!$response->isSuccessful()) {
            OnlinePaymentsPrestaShopUtility::dieJsonArray(['success' => \false, 'message' => $this->module->l('An error occurred while fetching the payment status.', 'payment')]);
        }
        $cart = new \Cart((int) $response->getPaymentTransaction()->getMerchantReference());
