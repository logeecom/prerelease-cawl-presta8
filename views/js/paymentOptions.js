...
const htpPrototype = function (e) {
   ...
   * @returns {null|string}
   */
  const getSurchargeTranslation = (lang = 'en') => {
    if (surchargeTranslation[lang]) {
      return surchargeTranslation[lang];
    }
...
//adding this code to make sure that submit button stays disabled in case when credit card information is not valid
  ...
  this.init = function () {
    ...
    if (true === this.dynamicSurcharge && 1 === this.surchargeEnabled) {
      ...
      let paymentStep = document.querySelector('#checkout-payment-step');
      let paymentMethodsForm = paymentStep.querySelector('.content');
      let existingInfo = paymentMethodsForm.querySelector('.op-surcharge-info');

      if (!existingInfo) {
        let completelyPaid = document.createElement('article');
        completelyPaid.classList.add('alert', 'alert-warning', 'op-surcharge-info');
        let messageBox = document.createElement('ul');
        let message = document.createElement('li');
        const langCode = document.documentElement.lang;
        const [iso] = langCode.split(/[-_]/);
        message.innerText = getSurchargeTranslation(iso);
