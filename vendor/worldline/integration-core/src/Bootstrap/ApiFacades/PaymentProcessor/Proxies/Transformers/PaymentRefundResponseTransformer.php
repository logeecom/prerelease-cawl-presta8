...
class PaymentRefundResponseTransformer
{
    ...
    {
        if (
            null === $refund->getRefundOutput() ||
            null === $refund->getStatusOutput() ||
            null === $refund->getRefundOutput()->getOperationReferences() ||
            null === $refund->getRefundOutput()->getOperationReferences()->getMerchantReference()
        ) {
        ...
        return new PaymentRefund(
            StatusCode::parse((int)$refund->getStatusOutput()->getStatusCode()),
            Amount::fromInt(
                $refund->getRefundOutput()->getAmountOfMoney()->getAmount(),
                Currency::fromIsoCode($refund->getRefundOutput()->getAmountOfMoney()->getCurrencyCode())
            )
        );
    }
}