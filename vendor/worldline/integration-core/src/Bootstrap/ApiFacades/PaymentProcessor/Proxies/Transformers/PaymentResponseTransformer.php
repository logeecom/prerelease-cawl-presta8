...
class PaymentResponseTransformer
{
    ...
    {
        if (
            null === $payment->getPaymentOutput() ||
            null === $payment->getStatusOutput() ||
            null === $payment->getPaymentOutput()->getReferences() ||
            null === $payment->getPaymentOutput()->getReferences()->getMerchantReference()
        ) {
        ...
        return new Payment(
            ...
            Amount::fromInt(
                ...
                Currency::fromIsoCode($payment->getPaymentOutput()->getAmountOfMoney()->getCurrencyCode())
            ),
            self::getToken($payment->getPaymentOutput()),
            $payment->getStatus(),
            $productId,
            $paymentMethodName
        );
    ...
    {
        if ($paymentOutput->getCardPaymentMethodSpecificOutput() &&
            !empty($paymentOutput->getCardPaymentMethodSpecificOutput()->getToken())
        ) {
            return $paymentOutput->getCardPaymentMethodSpecificOutput()->getToken();
        }

        if ($paymentOutput->getRedirectPaymentMethodSpecificOutput() &&
            !empty($paymentOutput->getRedirectPaymentMethodSpecificOutput()->getToken())
        ) {
    ...
    private static function getPaymentMethodLabel(string $productId, PaymentOutput $paymentOutput): string
    {
        ...
        return array_key_exists($productId, PaymentMethodDefaultConfigs::PAYMENT_METHOD_CONFIGS) ?
            PaymentMethodDefaultConfigs::PAYMENT_METHOD_CONFIGS[$productId]['name']['translation']
