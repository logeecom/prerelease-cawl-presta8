...
class CardsSettingsEntity extends Entity
{
    ...
    public function inflate(array $data) : void
    {
        ...
        $this->cardsSettings = new CardsSettings($cardsSettings['enable3ds'], $cardsSettings['enforceStrongAuthentication'], $cardsSettings['enable3dsExemption'], !empty($cardsSettings['exemptionType']) ? ExemptionType::fromState($cardsSettings['exemptionType']) : null, !empty($cardsSettings['exemptionLimit']) ? Amount::fromArray($cardsSettings['exemptionLimit']) : null);
    }
    public function toArray() : array
    {
        ...
        $data['mode'] = $this->mode;
        $data['cardsSettings'] = ['enable3ds' => $this->cardsSettings->isEnable3ds(), 'enforceStrongAuthentication' => $this->cardsSettings->isEnforceStrongAuthentication(), 'enable3dsExemption' => $this->cardsSettings->isEnable3dsExemption(), 'exemptionType' => $this->cardsSettings->getExemptionType() ? $this->cardsSettings->getExemptionType()->getType() : '', 'exemptionLimit' => $this->cardsSettings->getExemptionLimit() ? $this->cardsSettings->getExemptionLimit()->toArray() : ''];
        return $data;
    }
