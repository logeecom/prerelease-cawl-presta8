...
class MealvouchersCartProvider implements CartProvider
{
    ...
    public function get(): Cart
    {
        ...
        $mealvouchersCart = new Cart(
            $cart->getMerchantReference(),
            $cart->getTotal(),
            $cart->getTotalInEUR(),
            $cart->getCustomer(),
            $this->mergeLineItems($cart->getLineItems()),
            $cart->getShipping(),
            $cart->getDiscount()
        );
    ...
    private function mergeLineItems(LineItemCollection $lineItems): LineItemCollection
    {
        ...
        return new LineItemCollection([new LineItem(
            new Product(
                $this->getMergedProductId($lineItems),
                $this->getMergedProductName($lineItems, $assignedProductTypes),
                $this->getMergedProductCode($lineItems),
                $assignedProductTypes,
                $this->getMergedProductUnit($lineItems)
            ),
            $this->getMergedUnitPrice($lineItems),
            1,
            $this->getMergedDiscounts($lineItems)
        )]);
    }
