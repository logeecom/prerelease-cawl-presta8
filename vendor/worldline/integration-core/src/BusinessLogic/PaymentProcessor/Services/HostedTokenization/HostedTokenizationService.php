...
use OnlinePayments\Core\BusinessLogic\Domain\HostedTokenization\TokenResponse;
...
class HostedTokenizationService
{
    ...
    public function getValidTokens(CartProvider $cartProvider): ?ValidTokensResponse
    {
        ...
        $savedTokens = $this->tokensRepository->getForCustomer(
            $cartProvider->get()->getCustomer()->getMerchantCustomerId()
        );
        ...
        foreach ($savedTokens as $savedToken) {
            if (in_array($savedToken->getTokenId(), $hostedTokenization->getInvalidTokens(), true)) {
        ...
        return new ValidTokensResponse(
            $hostedTokenization,
            $validTokens
        );
    }
    ...
    public function pay(PaymentRequest $paymentRequest): PaymentResponse
    {
        ...
        if (null !== $paymentRequest->getTokenId()) {
            $token = $this->tokensRepository->get(
                $paymentRequest->getCartProvider()->get()->getCustomer()->getMerchantCustomerId(),
                $paymentRequest->getTokenId()
            );
        }

        $paymentResponse = $this->paymentsProxy->create(
            $paymentRequest, $this->getCardsSettings(), $this->getPaymentSettings(), $token
        );
        ...
        if (null === $paymentResponse->getRedirectUrl()) {
            $this->waitPaymentOutcomeProcess->startInBackground(
                $paymentResponse->getPaymentTransaction()->getPaymentId(),
                $paymentResponse->getPaymentTransaction()->getReturnHmac(),
            );
        }
    ...
    public function getTokens(string $customerId): array
    {
        ...
        foreach ($tokens as $token) {
            $result[] = new TokenResponse(
                ...
                $token->getCardNumber(),
                $token->getExpiryDate(),
                $this->logoUrlService->getLogoUrl($token->getProductId())
            );
        }
    ...
    public function deleteToken(string $customerId, string $tokenId): void
    {
        ...
        if (!$token) {
            throw new TokenNotFoundException(
                new TranslatableLabel(
                    'Token with provided id not found',
                    'token.notFound'
                )
            );
        }
        ...
        try {
            ...
            throw new TokenDeletionFailureException(
                new TranslatableLabel(
