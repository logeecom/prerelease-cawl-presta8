...
namespace OnlinePayments\Core\BusinessLogic\PaymentProcessor\Services\Payment;
...
class StatusUpdateService
{
    ...
    public function __construct(
        ...
        HostedTokenizationProxyInterface $hostedTokenizationProxy,
    ...
    public function updateOrderStatus(PaymentId $paymentId, ?string $returnHmac = null): void
    {
        ...
        if ($paymentDetails->getStatusCode()->equals(StatusCode::authorized())) {
            ...
            if ($paymentSettings &&
                $paymentSettings->getPaymentAction()->equals(PaymentAction::authorize()) &&
                !$paymentSettings->getAutomaticCapture()->equals(AutomaticCapture::never()) &&
                $paymentTransaction->getCaptureAt() === null
            ) {
        ...
        if ($paymentDetails->getStatusCode()->isRefunded()
            || $paymentDetails->getAmounts()->getRefundedAmount()->getPriceInCurrencyUnits() > 0) {
    ...
    private function shouldTryOrderCreation(
        PaymentTransaction $paymentTransaction,
        PaymentDetails $paymentDetails
    ): bool {
        if (
            $paymentDetails->getStatusCode()->equals(StatusCode::authorized()) ||
            $paymentDetails->getStatusCode()->equals(StatusCode::completed())
        ) {
            return true;
        }

        if (
            $paymentDetails->getStatusCode()->isCanceledOrRejected() ||
            $paymentDetails->getStatusCode()->isRefunded() ||
            $paymentDetails->getStatusCode()->equals(StatusCode::incomplete())
        ) {
            return false;
        }
    ...
    {
        return new WaitPaymentOutcome(
            $paymentTransaction,
            $this->isWaitingTimeExceeded($paymentTransaction)
        );
    }
    ...
    {
        if (
            empty($paymentTransaction->getCustomerId()) ||
            null === $paymentDetails->getTokenId()
        ) {
            return;
        }

        $storedToken = $this->tokensRepository->get($paymentTransaction->getCustomerId(),
            $paymentDetails->getTokenId());
        if (null !== $storedToken) {
            return;
        }

        $token = $this->hostedTokenizationProxy->getToken(
            $paymentTransaction->getCustomerId(),
            $paymentDetails->getTokenId()
        );
