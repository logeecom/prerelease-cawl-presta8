...
class WaitPaymentOutcomeProcess
{
    ...
    public function getPaymentOutcome(?PaymentId $paymentId, ?string $returnHmac = null, ?string $merchantReference = null): WaitPaymentOutcome
    {
        $paymentTransaction = $merchantReference
            ? $this->getPaymentTransactionByMerchantReference($merchantReference)
            : $this->getPaymentTransaction($paymentId, $returnHmac);
    ...
    private function getPaymentTransactionByMerchantReference(string $merchantReference): PaymentTransaction
    {
        ...
        if (!$paymentTransaction) {
            throw new PaymentTransactionNotFoundException(
                new TranslatableLabel(
                    "Payment transaction for merchant reference $merchantReference not found.",
                    'PaymentProcessor.paymentTransactionNotFound',
                    [$merchantReference]
                )
            );
