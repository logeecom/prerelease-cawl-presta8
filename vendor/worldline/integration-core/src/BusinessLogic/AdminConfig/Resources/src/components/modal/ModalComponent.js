...
(function () {
    ...
    function ModalComponent(configuration) {
        const { templateService, translationService, utilities, elementGenerator } = OnlinePaymentsFE,
        ...
        this.open = () => {
            ...
            modal = OnlinePaymentsFE.elementGenerator.createElementFromHTML(modalTemplate);
            ...
            if (config.title) {
                title.innerHTML = OnlinePaymentsFE.sanitize(translationService.translate(config.title));
            } else {
    ...
    OnlinePaymentsFE.components.Modal = {
