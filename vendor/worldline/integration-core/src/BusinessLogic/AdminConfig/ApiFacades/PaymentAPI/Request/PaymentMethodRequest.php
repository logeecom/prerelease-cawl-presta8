...
class PaymentMethodRequest extends Request
{
    ...
    public function __construct(
        ...
        ?array  $vaultTitles = [],
        ?string $logo = null,
        ?bool   $enableGroupCards = null,
        ?string $paymentOption = null,
        ?int    $sessionTimeout = null,
        ?string $intersolveProductId = null,
        ?string $recurrenceType = null,
        ?string $signatureType = null,
        ?bool   $instantPayment = null
    )
    {
    ...
    public function transformToDomainModel(): object
    {
        ...
        if (PaymentProductId::hostedCheckout()->equals($this->productId)) {
            $additionalData = new HostedCheckout(
                $this->logo ?? '',
                $this->enableGroupCards ?? true
            );
        }

        if (PaymentProductId::intersolve()->equals($this->productId)) {
            $additionalData = new Intersolve(
                 new Intersolve\SessionTimeout($this->sessionTimeout ?: 180),
                Intersolve\PaymentProductId::parse($this->intersolveProductId ?: '5700')
            );
        }

        if (PaymentProductId::oney3x()->equals($this->productId) ||
            PaymentProductId::oney4x()->equals($this->productId) ||
            PaymentProductId::oneyFinancementLong()->equals($this->productId) ||
            PaymentProductId::oneyBrandedGiftCard()->equals($this->productId) ||
            PaymentProductId::oneyBankCard()->equals($this->productId)) {
            $additionalData = new Oney($this->paymentOption ?? '');
        }

        if (PaymentProductId::sepaDirectDebit()->equals($this->productId)) {
            $additionalData = new Sepa(
                $this->recurrenceType ? Sepa\RecurrenceType::parse($this->recurrenceType) : Sepa\RecurrenceType::unique(),
                isset($this->signatureType) ? Sepa\SignatureType::parse($this->signatureType) : Sepa\SignatureType::sms()
            );
        }
        ...
        return new PaymentMethod(
            PaymentProductId::parse($this->productId),
            $nameCollection,
            $this->enabled,
            $this->template,
            $additionalData
