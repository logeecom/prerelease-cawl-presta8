...
namespace OnlinePayments\Core\BusinessLogic\AdminConfig\ApiFacades\MonitoringAPI\Controller;

use Exception;
use OnlinePayments\Core\BusinessLogic\AdminConfig\ApiFacades\MonitoringAPI\Response\DownloadMonitoringLogsResponse;
use OnlinePayments\Core\BusinessLogic\AdminConfig\ApiFacades\MonitoringAPI\Response\DownloadWebhookLogsResponse;
use OnlinePayments\Core\BusinessLogic\AdminConfig\ApiFacades\MonitoringAPI\Response\MonitoringLogsResponse;
use OnlinePayments\Core\BusinessLogic\AdminConfig\ApiFacades\MonitoringAPI\Response\WebhookLogsResponse;
...
class LogsController
{
    ...
    public function getMonitoringLogs(int $pageNumber = 1, int $pageSize = 10, string $searchTerm = ''): MonitoringLogsResponse
    {
        ...
        return new MonitoringLogsResponse(
            $this->monitoringLogsService->getLogs($pageNumber, $pageSize, $searchTerm),
            $this->hasNextPage($pageNumber, $pageSize, $numberOfItems),
            ($pageNumber - 1) * $pageSize + 1,
            ($pageNumber * $pageSize > $numberOfItems) ? $numberOfItems : $pageNumber * $pageSize,
            $numberOfItems
        );
    }
    ...
    public function getWebhookLogs(int $pageNumber = 1, int $pageSize = 10, string $searchTerm = ''): WebhookLogsResponse
    {
        ...
        return new WebhookLogsResponse(
            $this->webhookLogsService->getLogs($pageNumber, $pageSize, $searchTerm),
            $this->hasNextPage($pageNumber, $pageSize, $numberOfItems),
            ($pageNumber - 1) * $pageSize + 1,
            ($pageNumber * $pageSize > $numberOfItems) ? $numberOfItems : $pageNumber * $pageSize,
            $numberOfItems
