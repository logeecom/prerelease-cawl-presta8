...
class ConnectionMode
{
    ...
    {
        if (!in_array($mode, [self::LIVE, self::TEST], true)) {
            throw new InvalidConnectionModeException(
                new TranslatableLabel(
                    sprintf('Connection mode is invalid %s.', $mode),
                    'connection.invalidConnectionMode',
                    [$mode]
                )
            );
        }
