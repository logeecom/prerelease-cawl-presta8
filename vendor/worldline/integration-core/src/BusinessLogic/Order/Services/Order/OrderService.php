...
namespace OnlinePayments\Core\BusinessLogic\Order\Services\Order;
...
use OnlinePayments\Core\BusinessLogic\Domain\Order\OrderAction;
use OnlinePayments\Core\BusinessLogic\Domain\Order\OrderDetails;
use OnlinePayments\Core\BusinessLogic\Domain\Order\OrderPayment;
...
class OrderService
{
    ...
    public function getDetails(string $merchantReference): OrderDetails
    {
        ...
        foreach ($paymentDetails->getOperations() as $operation) {
            if (!in_array($operation->getStatus(), [
                self::STATUS_PAYMENT_PENDING_CAPTURE,
                self::STATUS_PAYMENT_CAPTURED,
            ])) {
                continue;
            }
            ...
            $payments[$payment->getProductId()] = new OrderPayment(
                $operation->getId(),
                $paymentDetails->getStatus(),
                $operation->getAmount(),
                $paymentDetails->getPaymentSpecificOutput()->getSurchargeAmount(),
                $payment->getPaymentMethodName(),
                $payment->getProductId(),
                $paymentDetails->getPaymentSpecificOutput()->getFraudResult(),
                $paymentDetails->getPaymentSpecificOutput()->getThreeDsLiability(),
                $paymentDetails->getPaymentSpecificOutput()->getThreeDsExemptionType(),
            );
        }

        $notAvailableAmount = $paymentDetails->getAmounts()->getCapturedAmount()->getValue() +
            $paymentDetails->getAmounts()->getCaptureRequestedAmount()->getValue() +
            $paymentDetails->getAmounts()->getCancelledAmount()->getValue();

        $amountToCapture = $paymentDetails->getAmount()->getValue() - $notAvailableAmount;
        $capturableAmount = Amount::fromInt(
            !$paymentDetails->getStatusOutput()->isAuthorized() || $amountToCapture < 0 ? 0 : $amountToCapture,
            $paymentDetails->getAmount()->getCurrency()
        );

        $amountToRefund = $paymentDetails->getAmounts()->getCapturedAmount()->getValue() - (
            $paymentDetails->getAmounts()->getRefundedAmount()->getValue() +
            $paymentDetails->getAmounts()->getRefundRequestedAmount()->getValue()
        );
        $refundableAmount = Amount::fromInt(
            !$paymentDetails->getStatusOutput()->isRefundable() || $amountToRefund < 0 ? 0 : $amountToRefund,
            $paymentDetails->getAmount()->getCurrency()
        );

        $amountToCancel = $paymentDetails->getAmount()->getValue() - $notAvailableAmount;
        $cancellableAmount = Amount::fromInt(
            !$paymentDetails->getStatusOutput()->isCancellable() || $amountToCancel < 0 ? 0 : $amountToCancel,
            $paymentDetails->getAmount()->getCurrency()
        );

        return new OrderDetails(
            $this->getOrderAmount($payments),
            $payments,
            new OrderAction(
                $paymentDetails->getStatusOutput()->isAuthorized(),
                $paymentDetails->getAmounts()->getCapturedAmount(),
                $paymentDetails->getAmounts()->getCaptureRequestedAmount(),
                $capturableAmount,
            ),
            new OrderAction(
                $paymentDetails->getStatusOutput()->isRefundable(),
                $paymentDetails->getAmounts()->getRefundedAmount(),
                $paymentDetails->getAmounts()->getRefundRequestedAmount(),
                $refundableAmount,
            ),
            new OrderAction(
                $paymentDetails->getStatusOutput()->isCancellable(),
                $paymentDetails->getAmounts()->getCancelledAmount(),
                Amount::fromInt(0, $paymentDetails->getAmount()->getCurrency()),
                $cancellableAmount,
            ),
            $paymentDetails->getStatusOutput()->getErrors()
        );
    }
