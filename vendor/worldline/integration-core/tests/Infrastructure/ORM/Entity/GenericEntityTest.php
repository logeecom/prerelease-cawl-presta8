...
use OnlinePayments\Core\Infrastructure\ORM\Configuration\IndexColumn;
...
abstract class GenericEntityTest extends TestCase
{
    ...
    public function testEntityConfiguration(Entity $entity)
    {
        ...
        $this->assertInstanceOf("OnlinePayments\Core\Infrastructure\ORM\Configuration\IndexMap", $indexMap);

        foreach ($indexMap->getIndexes() as $key => $item) {
            $this->assertNotEmpty($item, "Index configuration for $key must not be empty.");
            $this->assertInstanceOf("OnlinePayments\Core\Infrastructure\ORM\Configuration\IndexColumn", $item);
