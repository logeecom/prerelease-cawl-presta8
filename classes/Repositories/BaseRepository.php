...
use CAWL\OnlinePayments\Core\Infrastructure\ORM\QueryFilter\QueryCondition;
...
class BaseRepository implements RepositoryInterface
{
    ...
    public function select(QueryFilter $filter = null) : array
    {
        ...
        $result = $this->getRecordsByCondition($typeCondition . (!empty($whereCondition) ? ' AND ' . $whereCondition : ''), $filter);
    ...
    public function save(Entity $entity) : int
    {
        ...
        $record['entity_type'] = pSQL($entity->getConfig()->getType());
        $result = \Db::getInstance()->insert($this->getDbName(), $record);
        if (!$result) {
            $message = 'Entity ' . $entity->getConfig()->getType() . ' cannot be inserted. Error: ' . \Db::getInstance()->getMsgError();
    ...
    public function update(Entity $entity, QueryFilter $queryFilter = null) : bool
    {
        ...
        if (null !== $queryFilter) {
            ...
            $groups = $this->buildConditionGroups($queryFilter, $indexMap);
            $queryParts = $this->getQueryParts($groups, $indexMap);
            $whereClause .= " AND ({$this->generateWhereStatement($queryParts)})";
        }
        $result = \Db::getInstance()->update($this->getDbName(), $record, $whereClause);
        if (!$result) {
    ...
    public function delete(Entity $entity) : bool
    {
        ...
        if (!$result) {
            Logger::logError('Could not delete entity ' . $entity->getConfig()->getType() . ' with ID ' . $entity->getId());
        }
        return $result;
    }
    /**
     * Translates database records to Online Payments entities.
     *
     ...
     *
     * @return string|null Index column in Online Payments entity table.
     */
    ...
    /**
     * Returns columns that should be in the result of a select query on Online Payments entity table.
     *
    ...
    protected function getQueryPart(QueryCondition $condition, array $indexMap) : string
    {
        ...
        if (!\in_array($condition->getOperator(), array(Operators::NULL, Operators::NOT_NULL), \true)) {
            if (\in_array($condition->getOperator(), array(Operators::NOT_IN, Operators::IN), \true)) {
    ...
    private function addCondition(QueryCondition $condition, array $indexMap) : string
    {
        ...
        if (\in_array($condition->getOperator(), [Operators::NOT_IN, Operators::IN], \true)) {
            $values = \array_map(function ($item) {
                if (\is_string($item)) {
                    return "'{$item}'";
                }
            ...
            $conditionValue = "'" . pSQL($conditionValue, \true) . "'";
        }
        return $columnName . ' ' . $condition->getOperator() . (!\in_array($condition->getOperator(), [Operators::NULL, Operators::NOT_NULL], \true) ? $conditionValue : '');
    }
    /**
     * Returns Online Payments entity records that satisfy provided condition.
     *
     * @param string $condition Condition in format: KEY OPERATOR VALUE
     * @param QueryFilter|null $filter Query filter object.
     *
     * @return array Array of Online Payments entity records.
     *
     * @throws QueryFilterInvalidParamException
     * @throws \PrestaShopDatabaseException
     */
    protected function getRecordsByCondition(string $condition, QueryFilter $filter = null) : array
    {
        $query = new \DbQuery();
        $query->select(\implode(',', $this->getSelectColumns()))->from(bqSQL($this->getDbName()))->where($condition);
     ...
     *
     * @param Entity $entity Online Payments entity object.
    ...
    {
        $record = ['data' => pSQL($this->serializeEntity($entity), \true)];
    ...
    {
        $jsonEntity = \json_decode($data, \true);
