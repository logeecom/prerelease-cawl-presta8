...
namespace CAWL\OnlinePayments\Classes\Services\PaymentLink;
...
class OrderProviderService implements CartProvider
{
    ...
    private function getLineItems(array $products, \Currency $cartCurrency) : LineItemCollection
    {
        ...
        foreach ($products as $product) {
            $lineItems->add(new LineItem(new Product((string) $product['id_product'], (string) $product['product_name'], (string) $product['reference'] ?: $product['unique_id']), TaxableAmount::fromAmounts(Amount::fromFloat((float) $product['unit_price_tax_excl'], Currency::fromIsoCode($cartCurrency->iso_code)), Amount::fromFloat((float) $product['unit_price_tax_incl'], Currency::fromIsoCode($cartCurrency->iso_code))), (int) $product['product_quantity']));
        }
        return $lineItems;
    }
    ...
    {
        return Amount::fromFloat($orderTotal, Currency::fromIsoCode($cartCurrency->iso_code));
    }
    private function getDiscountAmount(float $discountAmount, \Currency $cartCurrency) : Amount
    {
        return Amount::fromFloat($discountAmount, Currency::fromIsoCode($cartCurrency->iso_code));
    }
