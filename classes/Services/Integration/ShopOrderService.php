...
use OnlinePayments\Core\BusinessLogic\Domain\Integration\ShopOrderService as ShopOrderServiceInterface;
...
class ShopOrderService implements ShopOrderServiceInterface
{
    ...
    public function createShopOrder(PaymentTransaction $paymentTransaction, PaymentDetails $paymentDetails, string $newState): void
    {
        ...
        $this->module->validateOrder(
            (int)$paymentTransaction->getMerchantReference(),
            (int)$newState,
            (float)$paymentDetails->getAmount()->getPriceInCurrencyUnits(),
            $paymentMethodText,
            null,
            ['transaction_id' => $paymentTransaction->getPaymentId()->getTransactionId()],
            null,
            false,
            $cart->secure_key
        );
    ...
    public function updateStatus(PaymentTransaction $paymentTransaction, PaymentDetails $paymentDetails, string $newState): void
    {
        ...
        $history->changeIdOrderState((int)$newState, $orderId, true);
        $history->add();
    }
    ...
    private function updateRefundDetailsForOrder(Order $order, \Currency $currency, Amount $totalAmountToRefund): array
    {
        ...
        foreach ($order->getOrderDetailList() as $detail) {
            ...
            $orderDetailTotal = TaxableAmount::fromAmounts(
                Amount::fromFloat($orderDetail->total_price_tax_excl, Currency::fromIsoCode($currency->iso_code)),
                Amount::fromFloat($orderDetail->total_price_tax_incl, Currency::fromIsoCode($currency->iso_code))
            );
            $orderDetailRefunded = TaxableAmount::fromAmounts(
                Amount::fromFloat($orderDetail->total_refunded_tax_excl, Currency::fromIsoCode($currency->iso_code)),
                Amount::fromFloat($orderDetail->total_refunded_tax_incl, Currency::fromIsoCode($currency->iso_code))
            );
            ...
            if ($refundableAmount->getAmountInclTax()->getValue() >= $totalAmountToRefund->getValue()) {
                $amountToRefund = TaxableAmount::fromAmountInclTaxAndTaxRate(
                    $totalAmountToRefund,
                    $amountToRefund->getTaxRate()
                );
            }

            $quantityToAdd = $this->calculateQuantityToAdd(
                $orderDetail,
                (int)ceil(
                    $amountToRefund->getAmountInclTax()->getPriceInCurrencyUnits() / (float)$orderDetail->unit_price_tax_incl
                )
            );

            $this->updateOrderDetail(
                $orderDetail,
                $amountToRefund->getAmountInclTax()->getPriceInCurrencyUnits(),
                $amountToRefund->getAmountExclTax()->getPriceInCurrencyUnits(),
                $quantityToAdd
            );
    ...
    private function updateOrderSlips(
        Order $order, \Currency $currency, Amount $totalAmountToRefund, array $newOrderSlipDetails
    ): void {
        $productsAmountToRefund = TaxableAmount::fromAmounts(
            Amount::fromInt(0, Currency::fromIsoCode($currency->iso_code)),
            Amount::fromInt(0, Currency::fromIsoCode($currency->iso_code)),
        );
        ...
        $shippingAmountToRefund = TaxableAmount::fromAmounts(
            Amount::fromInt(0, Currency::fromIsoCode($currency->iso_code)),
            Amount::fromInt(0, Currency::fromIsoCode($currency->iso_code)),
        );
        ...
        if ($totalAmountToRefund->getValue() > 0) {
            $alreadyRefundedShippingAmount = $this->getRefundedShippingAmount($order, $currency);
            $shippingTotal = TaxableAmount::fromAmounts(
                Amount::fromFloat($order->total_shipping_tax_excl, Currency::fromIsoCode($currency->iso_code)),
                Amount::fromFloat($order->total_shipping_tax_incl, Currency::fromIsoCode($currency->iso_code))
            );

            $shippingAmountToRefund = $shippingTotal->minus(
                TaxableAmount::fromAmountInclTaxAndTaxRate($alreadyRefundedShippingAmount, $shippingTotal->getTaxRate())
            );
        }

        if ($shippingAmountToRefund->getAmountInclTax()->getValue() > $totalAmountToRefund->getValue()) {
            $shippingAmountToRefund = TaxableAmount::fromAmountInclTaxAndTaxRate(
                $totalAmountToRefund, $shippingAmountToRefund->getTaxRate()
            );
        }
        ...
        foreach ($newOrderSlipDetails as $newOrderSlipDetail) {
            ...
            $this->addOrderSlipDetail(
                $orderSlip->id,
                $newOrderSlipDetail['orderDetail'],
                $newOrderSlipDetail['quantityToAdd'],
                $amountToRefund->getAmountInclTax()->getPriceInCurrencyUnits(),
                $amountToRefund->getAmountExclTax()->getPriceInCurrencyUnits()
            );
        }
     ...
     * @throws PrestaShopDatabaseException
     */
    private function addOrderSlipDetail(
        int $orderSlipId,
        OrderDetail $orderDetail,
        int $quantity,
        float $amount,
        ...
        Db::getInstance()->insert('order_slip_detail', [
            ...
            'unit_price_tax_excl' => (float)$amountWithoutTax / $quantity,
            'unit_price_tax_incl' => (float)$amount / $quantity,
    ...
    {
        return $quantityRefunded + (int)$orderDetail->product_quantity_return +
        (int)$orderDetail->product_quantity_refunded <= (int)$orderDetail->product_quantity ?
            $quantityRefunded :
            (int)$orderDetail->product_quantity - (int)$orderDetail->product_quantity_return
    ...
    private function getIdByCartId(string $merchantReference): ?int
    {
        ...
        $dbQuery
            ->select('o.id_order')
            ->from('orders', 'o')
            ->where('o.id_cart = ' . pSQL((int)$merchantReference));
