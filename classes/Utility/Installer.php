...
class Installer
{
    private static array $controllers = ['Connection', 'State', 'Stores', 'Version', 'Payments', 'Language', 'GeneralSettings', 'OrderStatuses', 'Monitoring', 'Transaction'];
    private static array $orderStatuses = [['configKey' => 'PENDING_ORDER_STATUS_ID', 'color' => '#34209E', 'logo' => 'icon_PENDING_ORDER_STATUS.gif', 'template' => '', 'send_email' => 0, 'invoice' => 0, 'logable' => 0, 'deleted' => 0, 'names' => ['fr' => 'En attente de la confirmation de paiement', 'en' => 'Awaiting payment confirmation', 'es' => 'Esperando confirmación del pago', 'it' => 'In attesa della conferma di pagamento', 'nl' => 'In afwachting van betalingsbevestiging', 'de' => 'Warten auf Zahlungsbestätigung']], ['configKey' => 'AWAITING_CAPTURE_STATUS_ID', 'color' => '#3498D8', 'logo' => 'icon_AWAITING_CAPTURE_STATUS.gif', 'template' => '', 'send_email' => 0, 'invoice' => 0, 'logable' => 0, 'deleted' => 0, 'names' => ['fr' => 'En attente de la remise en banque du paiement', 'en' => 'Awaiting payment capture', 'es' => 'Esperando captura del pago', 'it' => 'In attesa di ricevere il pagamento', 'nl' => 'In afwachting van betalingsvastlegging', 'de' => 'Warten auf Zahlungserfassung']]];
    private static array $hooks = ['paymentOptions', 'displayAdminOrderMainBottom', 'displayPaymentByBinaries', 'displayPaymentTop', 'actionFrontControllerSetMedia', 'actionAdminControllerSetMedia', 'customerAccount', 'displayPDFInvoice', 'displayAdminProductsExtra', 'actionProductUpdate', 'actionOrderSlipAdd', 'actionOrderStatusUpdate', 'displayBackOfficeHeader', 'actionValidateOrder', 'actionOrderGridDefinitionModifier', 'actionOrderGridPresenterModifier'];
    private static array $deprecated_hooks = ['displayAdminOrderLeft'];
    private OnlinePaymentsModule $module;
    /**
     * @param OnlinePaymentsModule $module
     */
    ...
    public function addController(string $name, int $parentId = -1) : void
    {
        ...
        $tab->class_name = $this->module->getBrand()->getCode() . $name;
    ...
    {
        $this->createTable(\strtolower($this->module->getBrand()->getCode()) . '_' . BaseRepository::TABLE_NAME, 9);
        $this->createTable(\strtolower($this->module->getBrand()->getCode()) . '_' . QueueItemRepository::TABLE_NAME, 9);
        $this->createTable(\strtolower($this->module->getBrand()->getCode()) . '_' . MonitoringLogsRepository::TABLE_NAME, 9);
        $this->createTable(\strtolower($this->module->getBrand()->getCode()) . '_' . WebhookLogsRepository::TABLE_NAME, 9);
    ...
    {
        $this->dropTable(\strtolower($this->module->getBrand()->getCode()) . '_' . BaseRepository::TABLE_NAME);
        $this->dropTable(\strtolower($this->module->getBrand()->getCode()) . '_' . QueueItemRepository::TABLE_NAME);
        $this->dropTable(\strtolower($this->module->getBrand()->getCode()) . '_' . MonitoringLogsRepository::TABLE_NAME);
        $this->dropTable(\strtolower($this->module->getBrand()->getCode()) . '_' . WebhookLogsRepository::TABLE_NAME);
        $this->dropTable(\strtolower($this->module->getBrand()->getCode()) . '_' . TokensRepository::TABLE_NAME);
        $this->dropTable(\strtolower($this->module->getBrand()->getCode()) . '_' . ProductTypesRepository::TABLE_NAME);
        $this->dropTable(\strtolower($this->module->getBrand()->getCode()) . '_' . PaymentTransactionsRepository::TABLE_NAME);
    }
    /**
    ...
    {
        $configKey = $this->module->getBrand()->getCode() . '_' . $orderStatus['configKey'];
        $orderState = new OrderState(Configuration::getGlobalValue($configKey));
    ...
    private function disconnect()
    {
        try {
            ...
            foreach ($connectedStores as $store) {
                StoreContext::doWithStore($store->getStoreId(), function () use($disconnectService) {
    ...
    private function removeImages() : void
    {
        $connectedStores = $this->getStoreService()->getStores();
        foreach ($connectedStores as $store) {
            StoreContext::doWithStore($store->getStoreId(), function () {
                $this->doRemoveImages();
            });
        }
